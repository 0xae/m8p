...
struct ColumnGroup {
    ...
    // "vectors", "analytics", "graph_links"
    ColumnGroup(size_t size_mb, std::string r) 
        : engine(std::make_unique<TensorGraph>(size_mb)), role(r) {}
};

// A BigTable is a logical collection of ColumnGroups.
// It allows scatter-gather queries across different storage backends.
struct BigTable {
    std::string name;
    // Map: GroupName -> Storage Engine
